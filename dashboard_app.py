...
try:
    ...
    st.write(f"Parâmetros na URL ao carregar a página: `{st.query_params}`")
    needs_scroll = "scroll" in st.query_params
    st.write(f"A página precisa rolar? `{needs_scroll}`")
    ...
    # Vamos desativar a limpeza da URL temporariamente para o teste
    ...
    if df_atual.empty or df_15dias.empty:
        ...
        with tab1:
            ...
            st.markdown(f"<h3>Comparativo de Backlog: ...</h3>", unsafe_allow_html=True) # Omitido por brevidade
            st.dataframe(processar_dados_comparativos(df_atual_filtrado, df_15dias_filtrado)) # Simplificado

            if not df_aging.empty:
                st.markdown("---")
                st.subheader("Detalhar e Buscar Chamados")
                
                # --- NOVO: Bloco de Diagnóstico com Alertas ---
                if needs_scroll:
                    st.warning("Tentando executar o script de rolagem...")
                    js_code = f"""
                        <script>
                            alert('Passo 1: O script de rolagem foi chamado.');
                            setTimeout(function() {{
                                const element = document.getElementById('detalhar-e-buscar-chamados');
                                if (element) {{
                                    alert('Passo 2: Elemento encontrado! Rolando...');
                                    element.scrollIntoView({{ behavior: 'smooth', block: 'start' }});
                                }} else {{
                                    alert('ERRO: Elemento com ID "detalhar-e-buscar-chamados" NÃO foi encontrado.');
                                }}
                            }}, 1000);
                        </script>
                    """
                ...
                st.selectbox("Selecione uma faixa de idade para ver os detalhes:", options=ordem_faixas, key='faixa_selecionada')
                # ... (resto do código)
